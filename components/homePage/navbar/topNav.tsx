"use client";

import React from "react";
import { HugeiconsIcon } from "@hugeicons/react";
import { Call02Icon } from "@hugeicons/core-free-icons";

interface WhatsAppButtonProps {
  number: string; // e.g. "+880123456789"
}

const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({ number }) => {
  const whatsappLink = `https://wa.me/${number.replace(/\D/g, "")}`;

  return (
    <div className="flex items-center">
      {/* Animated text */}
      <div className="w-2/3 md:w-1/2 overflow-hidden">
      <span className="inline-block whitespace-nowrap animate-marquee">
        Hire your moving service from 250$
      </span>

      <style jsx>{`
        @keyframes marquee {
          0% {
            transform: translateX(200%);
          }
          100% {
            transform: translateX(-100%);
          }
        }
        .animate-marquee {
          animation: marquee 10s linear infinite;
        }
      `}</style>
    </div>

      {/* WhatsApp + Call buttons */}
      <span className="w-1/2 pr-[5px] md:pr-[50px] pt-2 flex items-end justify-end md:gap-2 mb-0 pb-0">
        <a
          href={whatsappLink}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-end text-right gap-1"
        >
          {/* WhatsApp SVG */}
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <g clipPath="url(#clip0_2018_3110)">
              <path
                d="M24 12C24 5.37258 18.6274 0 12 0C5.37258 0 0 5.37258 0 12C0 18.6274 5.37258 24 12 24C18.6274 24 24 18.6274 24 12Z"
                fill="#0DC143"
              />
              <path
                d="M17.3754 6.60575C16.0889 5.2814 14.3105 4.5625 12.4943 4.5625C8.63481 4.5625 5.53211 7.70305 5.56996 11.5246C5.56996 12.7354 5.91051 13.9085 6.47806 14.9679L5.49426 18.5625L9.16456 17.6166C10.1862 18.1841 11.3213 18.449 12.4564 18.449C16.2781 18.449 19.3808 15.3085 19.3808 11.4868C19.3808 9.63275 18.6619 7.89225 17.3754 6.60575ZM12.4943 17.276C11.4727 17.276 10.451 17.0111 9.58076 16.4814L9.35371 16.3679L7.15916 16.9354L7.72671 14.7787L7.57536 14.5517C5.91051 11.8652 6.70506 8.30845 9.42941 6.6436C12.1537 4.97871 15.6727 5.7733 17.3375 8.49765C19.0024 11.222 18.2078 14.7409 15.4835 16.4058C14.6132 16.9733 13.5537 17.276 12.4943 17.276ZM15.824 13.076L15.4078 12.8868C15.4078 12.8868 14.8024 12.622 14.424 12.4327C14.3862 12.4327 14.3483 12.3949 14.3105 12.3949C14.197 12.3949 14.1213 12.4328 14.0456 12.4706C14.0456 12.4706 14.0078 12.5084 13.4781 13.1138C13.4402 13.1895 13.3646 13.2274 13.2889 13.2274H13.251C13.2132 13.2274 13.1375 13.1896 13.0997 13.1517L12.9105 13.076C12.4943 12.8868 12.1159 12.6598 11.8132 12.3571C11.7375 12.2814 11.624 12.2057 11.5483 12.13C11.2835 11.8652 11.0186 11.5625 10.8294 11.2219L10.7916 11.1463C10.7537 11.1085 10.7537 11.0706 10.7159 10.995C10.7159 10.9193 10.7159 10.8436 10.7537 10.8057C10.7537 10.8057 10.9051 10.6165 11.0186 10.503C11.0943 10.4274 11.1321 10.3139 11.2078 10.2382C11.2835 10.1247 11.3213 9.9733 11.2835 9.8598C11.2456 9.6706 10.7916 8.649 10.6781 8.42195C10.6024 8.30845 10.5267 8.2706 10.4132 8.23275H10.2997C10.224 8.23275 10.1105 8.23275 9.99696 8.23275C9.92131 8.23275 9.84561 8.2706 9.76996 8.2706L9.73211 8.30845C9.65641 8.3463 9.58076 8.42195 9.50506 8.4598C9.42941 8.53545 9.39156 8.61115 9.31591 8.6868C9.05101 9.02735 8.89966 9.4436 8.89966 9.8598C8.89966 10.1625 8.97536 10.4652 9.08886 10.7301L9.12671 10.8436C9.46726 11.5625 9.92131 12.2057 10.5267 12.7733L10.6781 12.9247C10.7916 13.0382 10.9051 13.1139 10.9808 13.2274C11.7754 13.9085 12.6835 14.4004 13.7051 14.6652C13.8186 14.7031 13.97 14.703 14.0835 14.7409C14.197 14.7409 14.3483 14.7409 14.4619 14.7409C14.651 14.7409 14.8781 14.6652 15.0294 14.5896C15.1429 14.5139 15.2186 14.5139 15.2943 14.4382L15.37 14.3625C15.4456 14.2868 15.5213 14.249 15.597 14.1733C15.6727 14.0976 15.7483 14.0219 15.7862 13.9463C15.8619 13.7949 15.8997 13.6058 15.9375 13.4165C15.9375 13.3409 15.9375 13.2274 15.9375 13.1517C15.9375 13.1517 15.8997 13.1139 15.824 13.076Z"
                fill="white"
              />
            </g>
            <defs>
              <clipPath id="clip0_2018_3110">
                <rect width="24" height="24" fill="white" />
              </clipPath>
            </defs>
          </svg>
          <span className="text-gray-900">{number}</span>
        </a>

        <a
          href={`tel:${number}`}
          className="hidden md:flex items-end text-right ml-1"
        >
          <HugeiconsIcon icon={Call02Icon} />
          <span className="text-gray-900">{number}</span>
        </a>
      </span>
    </div>
  );
};

export default WhatsAppButton;
